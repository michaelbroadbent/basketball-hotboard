...
export default function BasketballHotboard() {
  // Data state
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const coachesRes = await fetch('/data/coaches_basketball.json');
        if (!coachesRes.ok) throw new Error('Failed to load coaches data');
        const coaches = await coachesRes.json();
        setCoachesData(coaches);
        
        const torvikRes = await fetch('/data/torvik_trank_2008_2026.json');
    ...
    <div style={{
      ...
      <footer style={{
        ...
        <p style={{ color: '#8892b0', fontSize: '0.75rem', marginBottom: '0.5rem' }}>
          Basketball Coaching Hotboard • T-Rank data from barttorvik.com • {coachesData.length.toLocaleString()} coaches
        </p>
      </footer>
    </div>
  );
